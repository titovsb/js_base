...
let postPlace = document.querySelector('.scrollCheck');
let counter = 0;
...
function init() {
    ...
    if (window.innerHeight === document.body.offsetHeight) {
        insertPosts();
        // init();      // without recursion works well.
    }
}
init();

document.addEventListener('scroll', function (event) {
    if (postPlace.getBoundingClientRect().top <= window.innerHeight) {
...
function insertPosts() {
    ...
    postPlace.insertAdjacentHTML('beforeend', getPostMarkup(counter));
}
