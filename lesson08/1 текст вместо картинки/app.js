...
function handleClick(clickedButtonEvent) {
    ...
    // const cardNode = ;
    // let cardNode = clickedButtonEvent.target.parentElement;
    let cardNode = clickedButtonEvent.target;

    // 3. создайте литерал объекта со следующими свойствами:
    const card = {
        wrap: cardNode.parentElement, // здесь элемент с классом .product
        img: cardNode.previousElementSibling, // здесь картинка внутри .product
        productName: cardNode.parentElement.firstElementChild, // здесь .productName, который внутри .product
...
function showMoreText(card) {
    ...
    let text = 'Lorem ipsum dolores as sent again'
    // 6.2 внутри .product есть .productName, после него вставляем div.desc и текстом из переменной из п. 6.1
    card.wrap.querySelector('.productName').insertAdjacentHTML('afterend', `<div class="desc">${text}</div>`)
